...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    //read a 4 byte register 2048 times, 
    unsigned int bytes_transferred; // change obviously
    float time_spent; // change obviously
    bytes_transferred = (4*2048);                               //4 bytes (32 bit register) read 2048 times
    time_spent = (8*pow(10, -9))*(stop_time-start_time);        //number of clocks times clock period
    throughput = (pow(10,-6)*(4*2048))/(time_spent);                 //bytes to Mbytes divided by time spent 
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Andrew Hicks - Custom Peripheral Demonstration\n\r");
